...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return {
        ...
        alternates: {
            canonical: `${siteUrl}/skill/${skill.slug}`,
        },
    };
}
...
function getInstallCommands(skill: ReturnType<typeof getSkillBySlug>) {
    ...
    if (isAntigravitySkill) {
        return {
            quick: `curl -fsSL ${siteUrl}/i/${skill.slug} | bash`,
            manual: [
                {
                    step: 1,
            ...
            <div className="max-w-3xl mx-auto px-6">
                ...
                <div className="mb-8">
                    ...
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        <a
                            href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(`${siteUrl}/skill/${skill.slug}`)}&text=${encodeURIComponent(`Check out ${skill.name} - an awesome AI skill!`)}`}
                            target="_blank"
                        ...
                        <a
                            href={`https://www.reddit.com/submit?url=${encodeURIComponent(`${siteUrl}/skill/${skill.slug}`)}&title=${encodeURIComponent(skill.name)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 transition-colors"
                        ...
                        <a
                            href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`${siteUrl}/skill/${skill.slug}`)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 transition-colors"
                        ...
                        <a
                            href={`mailto:?subject=${encodeURIComponent(`Check out ${skill.name}`)}&body=${encodeURIComponent(`I found this awesome AI skill: ${siteUrl}/skill/${skill.slug}`)}`}
                            className="w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 transition-colors"
